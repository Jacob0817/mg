    ...
    <v-layout row wrap>
      ...
      <v-flex xs12 md7  style="font-size:30px">
        ...
        >
          ...
          <v-stepper-step color="success" :complete="stepSwithValue > 2" step="2">眼睛的做法</v-stepper-step>

          <v-stepper-content step="2">
            <v-card color="grey lighten-1" class="mb-1" height="350px">
              <p>眼睛朝下，做口诀： (上右上) (底底) (下左上) (底底) (下下) </p>
              <v-divider light ></v-divider>
              <div>
                <v-btn  class="cube-color" color="white lighten-1">
                    第一步：找到眼睛，眼睛面对自己
                </v-btn>
                <v-btn @click="cubetwist(`X'`,false)" class="cube-color" color="yellow lighten-1">
                    第二步：眼睛朝下
                </v-btn>
                <v-btn @click="cubetwist(`RU'RDDR'URDDR'R'`,false)" class="cube-color" color="red lighten-1">
                    第三步：做一次口诀(点击演示)
                </v-btn>
                ...
                <v-divider light ></v-divider>
                <v-btn @click="cubetwist(`X'`)" class="cube-color" 
                :color="stepcounter>1?'white lighten-1':'green lighten-1'">眼睛朝下</v-btn>
                <br>
                ...
                :color="stepcounter>3?'white lighten-1':'blue lighten-1'">右</v-btn>
                <v-btn @click="cubetwist(`R`)" class="cube-color " 
                :color="stepcounter>4?'white lighten-1':'green lighten-1'">上</v-btn>

                  <v-divider style="height:30px;border-color:black" vertical ></v-divider>

                <v-btn @click="cubetwist(`D`)" class="cube-color" 
                :color="stepcounter>5?'white lighten-1':'red lighten-1'">底</v-btn>
                <v-btn @click="cubetwist(`D`)"  class="cube-color " 
                :color="stepcounter>6?'white lighten-1':'orange lighten-1'">底</v-btn>
                  <v-divider style="height:30px;border-color:black" vertical ></v-divider>
                <v-btn @click="cubetwist(`R'`)" class="cube-color" 
                :color="stepcounter>7?'white lighten-1':'yellow lighten-1'">下</v-btn>
                <v-btn @click="cubetwist(`U`)" class="cube-color" 
                :color="stepcounter>8?'white lighten-1':'blue lighten-1'">左</v-btn>
                <v-btn @click="cubetwist(`R`)"  class="cube-color " 
                :color="stepcounter>9?'white lighten-1':'green lighten-1'">上</v-btn>

                  <v-divider style="height:30px;border-color:black" vertical ></v-divider>

                <v-btn @click="cubetwist(`D`)" class="cube-color" 
                :color="stepcounter>10?'white lighten-1':'red lighten-1'">底</v-btn>
                <v-btn @click="cubetwist(`D`)"  class="cube-color " 
                :color="stepcounter>11?'white lighten-1':'orange lighten-1'">底</v-btn>

                    <v-divider style="height:30px;border-color:black" vertical ></v-divider> 

                <v-btn @click="cubetwist(`R'`)" class="cube-color" 
                :color="stepcounter>12?'white lighten-1':'red lighten-1'">下</v-btn>
                <v-btn @click="cubetwist(`R'`)" class="cube-color" 
                :color="stepcounter>13?'white lighten-1':'blue lighten-1'">下</v-btn>
                <br>
              </div>
            </v-card>
            ...
            <v-card color="grey lighten-1" row class="mb-1" height="200px">
              <v-layout justify-center row fill-height>
                <v-flex xs12 text-xs-left>
                  <v-btn @click="cubetwist(`X'`,false)" color="primary" class="cube-color-small">眼睛口诀(眼睛对自己):</v-btn>
                  <v-btn @click="cubetwist(`RU'R`,false)" color="success" class="cube-color-small">上右上</v-btn>
                  <v-btn @click="cubetwist(`DD`,false)" color="warning" class="cube-color-small">底底</v-btn>
                  <v-btn @click="cubetwist(`R'UR`,false)" color="error" class="cube-color-small">下左上</v-btn>
                  <v-btn @click="cubetwist(`DD`,false)" color="primary" class="cube-color-small">底底</v-btn>
                  <v-btn @click="cubetwist(`R'R'`,false)" color="success" class="cube-color-small">下下</v-btn>
                  <br>
                  <v-divider light ></v-divider>
                  <v-btn class="cube-color" color="grey lighten-1">
                      没眼睛情况:
                  </v-btn>
                  <v-btn @click="cubetwist(`X'RU'RDDR'URDDR'R'`,false)" class="cube-color" color="green lighten-1">
                      先做一遍眼睛
                  </v-btn>
                  <v-btn @click="cubetwist(`Z'`,false)" class="cube-color" color="blue lighten-1">
                      找到眼睛，调整
                  </v-btn>
                  <v-btn @click="cubetwist(`RU'RDDR'URDDR'R'`,false)" class="cube-color" color="red lighten-1">
                      再按眼睛做一遍
...
export default {
  ...
  data: () => ({
    ...
    formulaTest : {
      ...
      cubeStateBefore : classStateList.class8base,
      cubeStateOk : classStateList.class8ok,
      cubeStateSp : classStateList.class8special,
      cubeRunscript:{},
  ...
  methods: {
    ...
    iscubeok:function(cubename){
        ...
        flag2 = cubename.cubeStage.faces.up.yellow == 9
        if(flag1 && flag2) {
            // if front 2 corner same
            let frontStage = cubename.cubeStage.layer3cornerface.front
            let frontKeys = Object.keys(frontStage)
            if(frontKeys.length == 1 && frontStage[frontKeys[0]] == 2) {
                // if right 2 corner same
                let rightStage = cubename.cubeStage.layer3cornerface.right
                let rightKeys = Object.keys(rightStage)
                if(rightKeys.length == 1 && rightStage[rightKeys[0]] == 2) {
                    return true
                }else{
                  console.log('right corner not same')
                  return false
                }
            }else{
              console.log('front corner not same')
              return false
            }
        }else{
          console.log('面没有复原')
    ...
    resetCube1 :function(){
      this.formulaTest.cubeStateBefore = classStateList.class8base
      this.stepcounter = 1
    ...
    gotoStage3: function() {
      ...
      this.formulaTest.cubeStateBefore = classStateList.class8special
      this.cube.show = false
      this.$nextTick(() => {
            this.cube.show = true;//重建组件
        })
