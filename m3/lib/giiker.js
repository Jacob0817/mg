...
GiikerCube.prototype = {
    ...
    giikerMoveToAlgMove(face, amount) {
        if (amount == 9) {
            console.log("Encountered 9", face, amount);
            amount = 2;
        }
    ...
    addEventListener(eventtype,evnetname,listener) {
        this.listeners.push({
            type : eventtype,
            name : evnetname,
            func : listener
    ...
    removeEventListener(evnetname){
        ...
        for(var i=0;i<this.listeners.length;i++){
            if(that.listeners[i].name == evnetname){
