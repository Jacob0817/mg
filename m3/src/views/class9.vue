...
<template>
  <v-container fluid>
    ...
    <v-layout row wrap>
      ...
      <v-flex xs12 md7  style="font-size:30px;background-color:#1d2935">
         <v-stepper
        ...
        >
          ...
          <v-stepper-step color="success" :complete="stepSwithValue > 2" step="2">棱快做法</v-stepper-step>

          <v-stepper-content step="2">
            <v-card color="grey lighten-1" class="mb-1" height="350px">
              <p>黄色朝上做口诀： (上右上左) (上左上右) (下右下下) </p>
              ...
              <div>
                ...
                <v-btn @click="cubetwist(`RU'RURURU'R'U'R'R'`,false)" class="cube-color" color="red lighten-1">
                    第三步：做一次口诀(点击演示)
                </v-btn>
                ...
                :color="stepcounter>0?'white lighten-1':'green lighten-1'">找好方向</v-btn>
                <br>

                <v-btn @click="cubetwist(`R`)" class="cube-color" 
                :color="stepcounter>1?'white lighten-1':'orange lighten-1'">上</v-btn>
                <v-btn @click="cubetwist(`U'`)" class="cube-color" 
                :color="stepcounter>2?'white lighten-1':'blue lighten-1'">右</v-btn>
                <v-btn @click="cubetwist(`R`)" class="cube-color " 
                :color="stepcounter>3?'white lighten-1':'green lighten-1'">上</v-btn>
                <v-btn @click="cubetwist(`U`)" class="cube-color" 
                :color="stepcounter>4?'white lighten-1':'red lighten-1'">左</v-btn>

                  <v-divider style="height:30px;border-color:black" vertical ></v-divider>

                
                <v-btn @click="cubetwist(`R`)" class="cube-color " 
                :color="stepcounter>5?'white lighten-1':'green lighten-1'">上</v-btn>
                <v-btn @click="cubetwist(`U`)" class="cube-color" 
                :color="stepcounter>6?'white lighten-1':'red lighten-1'">左</v-btn>
                <v-btn @click="cubetwist(`R`)" class="cube-color" 
                :color="stepcounter>7?'white lighten-1':'orange lighten-1'">上</v-btn>
                <v-btn @click="cubetwist(`U'`)" class="cube-color" 
                :color="stepcounter>8?'white lighten-1':'blue lighten-1'">右</v-btn>

                  <v-divider style="height:30px;border-color:black" vertical ></v-divider>

                <v-btn @click="cubetwist(`R'`)" class="cube-color" 
                :color="stepcounter>9?'white lighten-1':'yellow lighten-1'">下</v-btn>
                <v-btn @click="cubetwist(`U'`)" class="cube-color" 
                :color="stepcounter>10?'white lighten-1':'yellow lighten-1'">右</v-btn>
                <v-btn @click="cubetwist(`R'`)" class="cube-color" 
                :color="stepcounter>11?'white lighten-1':'red lighten-1'">下</v-btn>
                <v-btn @click="cubetwist(`R'`)" class="cube-color" 
                :color="stepcounter>12?'white lighten-1':'blue lighten-1'">下</v-btn>
                <br>
              </div>
            </v-card>
            ...
            <v-card color="grey lighten-1" row class="mb-1" height="200px">
              <v-layout justify-center row fill-height>
                <v-flex xs12 text-xs-left>
                  <v-btn @click="cubetwist(`RU'RU`,false)" color="success" class="cube-color-small">上右上左</v-btn>
                  <v-btn @click="cubetwist(`RURU'`,false)" color="warning" class="cube-color-small">上左上右</v-btn>
                  <v-btn @click="cubetwist(`R'U'R'R'`,false)" color="error" class="cube-color-small">下右下下</v-btn>
                  <br>
                  <v-divider light ></v-divider>
                  <v-btn class="cube-color" color="grey lighten-1">
                      没完整一面:
                  </v-btn>
                  <v-btn @click="cubetwist(`RU'RURURU'R'U'R'R'`,false)" class="cube-color" color="green lighten-1">
                      先做一遍
                  </v-btn>
                  <v-btn @click="cubetwist(`Y`,false)" class="cube-color" color="blue lighten-1">
                      再调整
                  </v-btn>
                  <v-btn @click="cubetwist(`RU'RURURU'R'U'R'R'`,false)" class="cube-color" color="red lighten-1">
                      再做一遍
...
export default {
  ...
  data: () => ({
    ...
    formulaTest : {
      ...
      cubeStateBefore : classStateList.class9base,
      cubeStateOk : classStateList.class9ok,
      cubeStateSp : classStateList.class9special,
      cubeRunscript:{},
    }
  }),
  mounted: function() {
  ...
  methods: {
    ...
    // 重置几个魔方
    resetCube1 :function(){
      this.formulaTest.cubeStateBefore = classStateList.class9base
      this.stepcounter = 1
    ...
    gotoStage3: function() {
      ...
      this.formulaTest.cubeStateBefore = classStateList.class9special
      this.cube.show = false
      this.$nextTick(() => {
            this.cube.show = true;//重建组件
        })
